...
#
# 锁表搬迁为 GameCtx 实例字段的原因：模块级 dict 在热重载时会跨 loop 残留
# 旧的 asyncio.Lock 对象（锁底层绑定了创建时的 loop），下一次 initialize()
# 拿到的是全新 loop，旧锁会导致 "Future attached to a different loop" 异常
# 与跨进程 double-spend 窗口。实例字段跟 plugin 生命周期走，terminate() 清空。
...
def install(cls, flt, module_path: str, routes) -> int:
    ...
    锁表从模块级 dict 改为插件实例字段（`cls._player_locks`），卸载 / 热重载
    时由 main.terminate() 调 PlayerLockTable.clear()，避免跨 loop 残留。
    """
    installed = 0
    for route in routes:
...
def at_targets(event) -> list[str]:
    ...
    不做 isdigit 校验：QQ 官方平台的 uid 是含字母的 openid，旧写法会把所有
    @ 目标全部滤掉，导致转账/决斗等指令在该平台上永远取不到目标。
    """
...
def numbers(event) -> list[str]:
    ...
    return [n for n in re.findall(r"\d+", event.message_str or "") if len(n) <= MAX_ARG_LEN]
