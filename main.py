...
class SlaveMarketPlugin(Star):
    ...
    def load_copywriting(self) -> Dict[str, List[str]]:
        ...
        """
        copywriting_path = os.path.join(self.plugin_path, "resources", "data", "workCopywriting.json")
    ...
    def save_player_data(self, group_id: str, user_id: str, data: Dict[str, Any]) -> None:
        """保存玩家数据
    ...
    def ensure_player_exists(self, group_id: str, user_id: str, nickname: str = "") -> Dict[str, Any]:
        """确保玩家存在，如果不存在则创建
    ...
    def get_group_list(self) -> List[str]:
        ...
        if os.path.exists(player_dir):
            for dirname in os.listdir(player_dir):
                if os.path.isdir(os.path.join(player_dir, dirname)) and dirname.isdigit():
    ...
    def generate_market_text(self, data: Dict[str, Any]) -> str:
        ...
        if user.get("master"):
            master_data = self.get_player_data(str(user.get("group_id", "")), str(user["master"]))
            if master_data:
    ...
    async def work(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if not self.check_cooldown(data, "work", self.config["work"]["cooldown"]):
                remaining = self.config["work"]["cooldown"] - (int(time.time()) - data["cooldowns"]["work"])
            ...
            if is_slaveowner:
                ...
                work_descriptions = self.copywriting.get("slaveowner", ["完成工作获得收入"])
            else:
    ...
    async def purchase_slave(self, event: AstrMessageEvent, target_user: str):
        ...
        try:
            ...
            # 确保目标用户存在
            target_data = self.ensure_player_exists(group_id, target_id, f"用户{target_id}")
            
            # 检查冷却时间
            if not self.check_cooldown(buyer_data, "purchase", self.config["purchase"]["cooldown"]):
                remaining = self.config["purchase"]["cooldown"] - (int(time.time()) - buyer_data["cooldowns"]["purchase"])
            ...
            if buyer_data["currency"] < purchase_price:
                yield event.plain_result(f"金币不足！需要 {purchase_price} 金币，你只有 {buyer_data['currency']} 金币")
                return
            ...
            yield event.plain_result(f"✅ 成功购买奴隶 {target_data['nickname']}！\n💰 花费 {purchase_price} 金币")
