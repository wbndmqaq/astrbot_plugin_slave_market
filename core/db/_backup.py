        ...
        with self._lock:  # 命名全程持锁，杜绝并发撞名
            with self._inflight_guard():
                ...
                try:
                    ...
                    with contextlib.closing(self._connect()) as src:
                        with contextlib.closing(sqlite3.connect(dest, timeout=15)) as dst:
                            src.backup(dst)
                except Exception:
            ...
            with self._inflight_guard():
                ...
                if self.path.exists():
                    try:
                        ...
                        with contextlib.closing(self._connect()) as src_p:
                            with contextlib.closing(
                                sqlite3.connect(pre_path, timeout=15)
                            ) as dst_p:
                                src_p.backup(dst_p)
